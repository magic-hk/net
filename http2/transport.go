...
type Transport struct {
	...
	//
	// Receipt of a PUSH_PROMISE on a non-tls.Conn connection will result
...
func (rl *clientConnReadLoop) processPushPromise(f *MetaPushPromiseFrame) error {
	...
	if err := checkValidPushPromiseRequestHeaders(pushedReq.Header); err != nil {
		return StreamError{f.StreamID, ErrCodeProtocol, err}
	}
	if err := checkValidHTTP2RequestHeaders(pushedReq.Header); err != nil {
		return StreamError{f.StreamID, ErrCodeProtocol, err}
	}
	...
	if !skipVerify {
		...
		_, pushPort := authorityHostPort(pushedReq.URL.Scheme, pushedReq.URL.Host)
		_, origPort := authorityHostPort(stream.req.URL.Scheme, stream.req.URL.Host)
