...
func (fr *Framer) ReadFrame() (Frame, error) {
	...
	if fh.Type == FrameHeaders && fr.ReadMetaHeaders != nil {
		return fr.readHeadersListContinued(f.(*HeadersFrame), fh.StreamID)
	}
	if fh.Type == FramePushPromise && fr.ReadMetaHeaders != nil {
		return fr.readHeadersListContinued(f.(*PushPromiseFrame), fh.StreamID)
	}
	return f, nil
}
...
type metaFrame interface {
	...
	addHeaders(headers ...hpack.HeaderField)
	setTruncated()
}
...
func (mh *MetaHeadersFrame) addHeaders(headers ...hpack.HeaderField) {
	mh.Fields = append(mh.Fields, headers...)
}

func (mh *MetaHeadersFrame) setTruncated() {
	mh.Truncated = true
}
...
func (mp *MetaPushPromiseFrame) addHeaders(headers ...hpack.HeaderField) {
	mp.Fields = append(mp.Fields, headers...)
}

func (mp *MetaPushPromiseFrame) setTruncated() {
	mp.Truncated = true
}

func checkPseudos(mf metaFrame) error {
...
// readHeadersListContinued returns 0 or more CONTINUATION frames from fr and
// merges them into into the provided continuable and returns a frame with the
// decoded hpack values.
func (fr *Framer) readHeadersListContinued(cont continuable, streamID uint32) (Frame, error) {
	if fr.AllowIllegalReads {
	...
	switch f := cont.(type) {
	case *HeadersFrame:
		mf = &MetaHeadersFrame{HeadersFrame: f}
	case *PushPromiseFrame:
		mf = &MetaPushPromiseFrame{PushPromiseFrame: f}
	default:
		errText := fmt.Sprintf("frame %v not supported as headerListContinuedFrame", f)
	...
	if invalid != nil {
		...
		return nil, StreamError{streamID, ErrCodeProtocol, invalid}
	}
	if err := checkPseudos(mf); err != nil {
		fr.errDetail = err
		if VerboseLogs {
			log.Printf("http2: invalid pseudo headers: %v", err)
		}
		return nil, StreamError{streamID, ErrCodeProtocol, err}
	}
	return mf, nil
}
